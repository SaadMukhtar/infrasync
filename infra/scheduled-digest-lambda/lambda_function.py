...
async def fetch_active_monitors():
    ...
    async with httpx.AsyncClient() as client:
        response = await client.get(
            ...
            params={
                "select": "*",
                "deleted": "eq.false"  # Only non-deleted monitors
            }
        )
...
async def cleanup_deleted_monitors():
    ...
    async with httpx.AsyncClient() as client:
        ...
        logger.info(f"Found {len(monitors_to_delete)} deleted monitors to permanently remove")
        ...
        for monitor in monitors_to_delete:
            ...
            logger.info(f"Permanently deleting monitor for repo {monitor.get('repo', 'unknown')} (deleted at {deleted_at})")
        ...
        logger.info(f"Successfully permanently deleted {len(monitors_to_delete)} monitors")
    ...
    async with httpx.AsyncClient() as client:
        ...
        for monitor in monitors:
            if monitor["frequency"] not in ("daily", "weekly"):
                logger.debug(f"Skipping monitor {monitor.get('repo', 'unknown')} - frequency {monitor.get('frequency', 'unknown')} not scheduled")
                continue
            ...
            logger.info(f"Triggering digest for repo: {monitor['repo']} (delivery: {monitor['delivery_method']})")
            try:
                ...
                if resp.status_code == 200:
                    ...
                    logger.error(f"Failed to trigger digest for {monitor['repo']} - Status: {resp.status_code}")
                    failed_digests += 1
            except Exception as e:
                logger.error(f"Error triggering digest for {monitor['repo']}: {str(e)}")
                failed_digests += 1
        
        logger.info(f"Digest processing complete - Success: {successful_digests}, Failed: {failed_digests}")
