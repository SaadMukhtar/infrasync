...
const MonitorDetailPage = () => {
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .then((res) => res.json())
      .then((data) => {
        console.log("Fetched timeseries data:", data.timeseries);
  ...
  console.log("Current chartMetric:", chartMetric);
  if (timeseries.length > 0) {
    console.log("Sample timeseries point:", timeseries[0]);
  }
  ...
  // Defensive: always coerce to number for charting
  const chartHasData = timeseries.some((d) => Number(d[chartMetric] ?? 0) > 0);

  // Defensive: always coerce summary metrics to number
  const safeMetric = (metrics: MonitorMetrics | null, key: string) =>
    Number(metrics?.[key] ?? 0);
  const safePrevMetric = (metrics: MonitorMetrics | null, key: string) =>
    Number(metrics?.[key] ?? 0);
            ...
            <AlertDialog
              ...
              <AlertDialogTrigger asChild>
                <Button
                  variant="destructive"
                  size="sm"
                  className="flex items-center gap-2 w-full max-w-xs sm:max-w-none sm:w-auto mt-2 sm:mt-0">
        ...
        {monitor && (
          <>
            <Card className="p-6 mb-6">
              ...
              <TooltipProvider>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                  {METRIC_LABELS.map(({ key, label, desc }) => {
                    ...
                    const prev = safePrevMetric(prevMetrics, key);
                    const {
                      icon: TrendIcon,
                      color,
                      diff,
              ...
              <div className="flex items-center gap-2 mb-2 overflow-x-auto flex-nowrap sm:flex-wrap">
