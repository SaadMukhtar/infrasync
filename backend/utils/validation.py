    ...
    try:
        ...
        if delivery_method == "slack":
            ...
            return (
                ...
                parsed.netloc.endswith('.slack.com') and
...
def validate_monitor_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if 'webhook_url' in data and data['webhook_url']:
        if not validate_webhook_url(data['webhook_url'], data.get('delivery_method', '')):
...
def validate_user_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if 'username' in data:
        ...
        if not re.match(r'^[a-zA-Z0-9_-]{3,50}$', username):
            errors.append("Username must be 3-50 characters, alphanumeric, hyphens, underscores only")
...
def validate_pagination_params(page: int = 1, limit: int = 10) -> tuple:
