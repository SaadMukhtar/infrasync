...
class MonitorService:
    ...
    async def create_monitor(
        self, data: MonitorCreate, created_by: str, github_token: Optional[str] = None
    ) -> Monitor:
        # Check if repo is private
        is_private = False
        if github_token:
            try:
                is_private = await self.github_service.is_repo_private(
                    data.repo, github_token
                )
            except Exception as e:
                logger.error(f"Failed to check repo privacy: {e}")
                raise Exception(
                    "Failed to check if repo is private. Please check your GitHub token."
                )
        else:
        ...
        if is_private and not github_token:
            raise Exception(
                "A GitHub token is required to monitor private repositories."
            )
        ...
        monitor = Monitor(
            ...
            webhook_url=cast(HttpUrl, str(data.webhook_url)),
        ...
        insert_result = self.client.table("monitors").insert(monitor_dict).execute()
        if not insert_result or insert_result.data is None:
            raise Exception(f"Insert failed: {insert_result}")
        return monitor

    async def list_monitors(self, org_id: str) -> List[Monitor]:
        result = (
            self.client.table("monitors")
            .select("*")
            .eq("org_id", org_id)
    ...
    async def delete_monitor(
        ...
        if not result or not result.data:
            logger.warning(
                f"Monitor {monitor_id} not found for deletion in org {org_id}"
            )
            return None
        monitor_data: Dict[str, Any] = result.data
    ...
    async def update_monitor_frequency(
        self, monitor_id: str, new_freq: str, org_id: str
    ) -> bool:
        result = (
            self.client.table("monitors")
        ...
        if result and result.data:
            logger.info(
                f"Updated monitor {monitor_id} frequency to {new_freq} for org {org_id}"
            )
            return True
