...
async def get_or_create_user(github_id: str, username: str, email: str, access_token: str) -> Optional[Dict[str, Any]]:
    ...
    if resp.data and len(resp.data) > 0:
        ...
        supabase.table("users").update(
            {"username": username, "email": email, "access_token": encrypted_token}
    ...
    if insert_resp.data and isinstance(insert_resp.data[0], dict):
        return insert_resp.data[0]
    return None


async def get_user_org_and_role(user_id: str) -> Tuple[Optional[str], Optional[str]]:
    # Return the first org the user belongs to, only if both user_orgs and org are not deleted
    resp = (
        supabase.table("user_orgs")
...
def delete_user_and_data(user_id: str) -> None:
    ...
    try:
        ...
        print(
            f"Starting deletion process for user {user_id} ({user_data.get('email', 'no email')})"
        )
        ...
        for membership in memberships.data or []:
            ...
            if role == "admin":
                ...
                if admin_count.count == 0:
                    ...
                    print(
                        f"User {user_id} is the only admin of org {org_id}, will delete org"
                    )
                    ...
                    print(
                        f"Removing admin user {user_id} from org {org_id} (other admins exist)"
                    )
            ...
            if role == "admin":
                ...
                if admin_count.count == 0:
                    validation_results.append(
                        f"WARNING: User is only admin of org {org_id} - org will be deleted"
                    )
                else:
                    validation_results.append(
                        f"OK: User is admin of org {org_id} but {admin_count.count} other admins exist"
                    )
            else:
                validation_results.append(
                    f"OK: User is {role} in org {org_id} - safe to remove"
                )
