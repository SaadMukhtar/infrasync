    ...
    try:
        ...
        if not monitor:
            ...
            digest_service.log_digest(
                ...
                created_by="",  # Optionally set if you have user context
                raw_payload=None,  # Optionally log raw data
                metrics_json=metrics,
            )
            # Log all monitors for this repo for debugging
            logger.error(
                f"No monitor found for repo={body.repo} webhook_url={body.webhook_url}. Fetching all monitors for this repo for debugging."
            )
            try:
                all_monitors = (
                    monitor_service.client.table("monitors")
                    .select("id, repo, webhook_url")
                    .eq("repo", body.repo)
                    .execute()
                )
                logger.error(
                    f"Monitors in DB for repo={body.repo}: {all_monitors.data if all_monitors and all_monitors.data else 'None'}"
                )
