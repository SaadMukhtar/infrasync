...
async def get_org_context(jwt_token: str = Cookie(None)) -> dict[str, Any]:
...
async def create_monitor(
    ...
    github_token = await get_user_github_token(user_id)  # type: ignore
    if github_token is None:
        github_token = ""
    try:
        monitor = await service.create_monitor(
            config, created_by=user_id, github_token=github_token
        )
        # Audit log
...
async def update_monitor_frequency(
    ...
    updated = await service.update_monitor_frequency(
        monitor_id, new_freq, str(org["org_id"])
    )
    if not updated:
...
async def get_monitor_digests(
    ...
    if (
        not monitor_result
        or not monitor_result.data
        or not isinstance(monitor_result.data, dict)
        or "org_id" not in monitor_result.data
        or str(monitor_result.data["org_id"]) != str(org["org_id"])
    ):
        raise HTTPException(status_code=404, detail="Monitor not found")
    digests: List = digest_service.get_monitor_digests(monitor_id, limit)  # type: ignore
