...
async def create_portal_session(
    ...
    logger.info(f"(billing/create): Creating portal session for org {org_id}")
    org = await get_org_by_id(org_id)
    if not org.get("stripe_customer_id"):
        logger.error(f"(billing/create): No Stripe customer for org {org_id}")
    ...
    try:
        ...
        logger.error(f"(billing/create):Stripe error: {e.user_message}")
        raise HTTPException(500, f"Stripe error: {e.user_message}")
    except Exception as e:
        logger.error(f"(billing/create): Error creating portal session: {e}")
        raise HTTPException(500, f"Error creating portal session: {e}")
    return {"url": session.url}
