...
from config import STRIPE_SECRET_KEY, STRIPE_WEBHOOK_SECRET, STRIPE_PRICE_IDS, FRONTEND_URL, PLAN_LIMITS
import stripe
...
# --- Dependencies ---
def get_jwt_payload(jwt_token: str = Cookie(None)):
    payload = verify_jwt_token(jwt_token)
    if not payload:
        raise HTTPException(status_code=401, detail="Not authenticated")
    return payload

def require_org_admin(payload=Depends(get_jwt_payload)):
    if payload.get("role") != "admin":
        raise HTTPException(status_code=403, detail="Admin access required")
    return payload

def require_org_member(payload=Depends(get_jwt_payload)):
...
async def get_org_by_id(org_id: str) -> Any:
    ...
    org_data = org_service.supabase.table("organizations").select("*", count="exact").eq("id", org_id).eq("deleted", False).single().execute()
    if not org_data.data:
        raise HTTPException(status_code=404, detail="Org not found")
    org = org_data.data
    # Add monitor count for enforcement (only non-deleted monitors)
    monitor_count = org_service.supabase.table("monitors").select("id", count="exact").eq("org_id", org_id).eq("deleted", False).execute().count or 0
    org["monitor_count"] = monitor_count
    return org

async def update_org_billing(org_id: str, **fields):
    # Update org billing fields in Supabase
    update = org_service.supabase.table("organizations").update(fields).eq("id", org_id).execute()
    if update.error:
        raise HTTPException(status_code=500, detail=f"Failed to update billing: {update.error}")
    return update.data
...
async def upgrade_subscription(org_id: str = Body(...), plan: str = Body(...), payload=Depends(require_org_admin)):
    ...
    # If no subscription, create one
        ...
        try:
            ...
            if not customer_id:
                customer = stripe.Customer.create(email=payload["email"], metadata={"org_id": org_id})
            ...
            await update_org_billing(org_id, plan=plan, billing_enabled=True, stripe_subscription_id=subscription.id)
...
async def create_checkout_session(org_id: str, plan: str, payload=Depends(require_org_admin)):
    org = await get_org_by_id(org_id)
    if org.get("is_internal"):
        raise HTTPException(403, "Internal orgs are free")
    if plan not in STRIPE_PRICE_IDS:
        raise HTTPException(400, "Invalid plan")
    if org.get("stripe_subscription_id"):
        raise HTTPException(400, "Org already has a subscription. Use the upgrade endpoint or Stripe portal.")
...
async def create_portal_session(org_id: str, payload=Depends(require_org_admin)):
    org = await get_org_by_id(org_id)
    if not org.get("stripe_customer_id"):
...
async def stripe_webhook(request: Request):
    ...
    if event["type"] == "checkout.session.completed":
        ...
        orgs = org_service.supabase.table("organizations").select("id").eq("stripe_customer_id", customer_id).eq("deleted", False).execute()
        if orgs.data and len(orgs.data) > 0:
            ...
            if subscription_id:
                ...
                plan = next((k for k, v in STRIPE_PRICE_IDS.items() if v == price_id), "free")
                print(f"Updating org {org_id} to plan {plan}")
                await update_org_billing(org_id, plan=plan, billing_enabled=True, stripe_subscription_id=subscription_id)
        ...
        if orgs.data and len(orgs.data) > 0:
            org_id = orgs.data[0]["id"]
            print(f"Found org ID: {org_id}")
            await update_org_billing(org_id, plan="free", billing_enabled=False, stripe_subscription_id=None)
            print(f"Successfully updated org {org_id} to free plan")

    return {"status": "ok"}

@router.get("/status")
async def billing_status(org_id: str, payload=Depends(require_org_member)):
    org = await get_org_by_id(org_id)
    return {
        "plan": org["plan"],
...
@router.post("/refresh")
async def refresh_billing_status(org_id: str, payload=Depends(require_org_member)):
