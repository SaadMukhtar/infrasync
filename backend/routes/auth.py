...
from utils.validation import get_cookie_domain
import logging
...
@router.get("/me")
def get_me(request: Request, jwt_token: Optional[str] = Cookie(None)) -> dict[str, Any]:
...
def logout(response: Response) -> dict[str, str]:
    response.delete_cookie(
        key="jwt_token", path="/", domain=COOKIE_DOMAIN if COOKIE_DOMAIN else None
    )
