...
import os
import httpx
from services.user import get_or_create_user, get_user_org_and_role, delete_user_and_data, get_user_by_id
...
GITHUB_OAUTH_REDIRECT_URI = os.getenv("GITHUB_OAUTH_REDIRECT_URI", "http://localhost:8000/api/v1/auth/github/callback")
...
async def github_callback(request: Request, code: str = None):
    ...
    async with httpx.AsyncClient() as client:
        ...
        # Get org context
        org_id, role = await get_user_org_and_role(user["id"])
        if not org_id:
...
def set_jwt_cookie(data: dict, response: Response):
...
def get_me(jwt_token: str = Cookie(None)):
    ...
    if org_id and user_id:
        ...
        logger.info(f"(auth/me): org_service.is_user_in_org({org_id}, {user_id}) -> {is_member}")
        if not is_member:
            # User is no longer a member, update JWT to remove org context
            logger.info(f"User {user_id} is no longer a member of org {org_id}, updating JWT")
            updated_payload = {
                "sub": user_id,
    ...
    return {
        ...
        "needs_org_setup": payload.get("needs_org_setup", False)
    }
...
def delete_own_account(response: Response, jwt_token: str = Cookie(None)):
    payload = verify_jwt_token(jwt_token)
...
def cleanup_deleted_users(days_old: int = Query(30, description="Delete users older than N days"), jwt_token: str = Cookie(None)):
...
def validate_account_deletion(jwt_token: str = Cookie(None)):
...
async def validate_github_repo(repo: str = Query(...), jwt_token: str = Cookie(None)):
    payload = verify_jwt_token(jwt_token)
    if not payload or not payload.get("sub"):
        raise HTTPException(status_code=401, detail="Not authenticated")
    ...
    try:
        ...
        headers = {"Authorization": f"token {decrypted_token}", "Accept": "application/vnd.github.v3+json"}
